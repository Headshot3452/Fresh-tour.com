<?php
    class AskanswerController extends ModuleController
    {
        ...
        public function getLeftMenu()
        {
            ...
            return array_merge(
                    array(
                        array('text' => CHtml::link('<img class="root-folder-orange" src="/images/icon-admin/folder-orange.png">'.Yii::t('app','Create root category'),array('create_category')),'children'=>array())
        ...
        public function UrlTopPagination($count_item)
        {
            ...
            if($count_page>1)
            {
                $prev = (isset($_GET['AskAnswer_page']) && $_GET['AskAnswer_page']>1)?$_GET['AskAnswer_page']-1:'1';
                $link_prev = $this->createUrl('answers').'?category_id='.$this->_active_category_id.'&AskAnswer_page='.$prev;

                if(isset($_GET['AskAnswer_page']))
                    $next = ($_GET['AskAnswer_page']<$count_page)?$_GET['AskAnswer_page']+1:$count_page;
                else
                    $next = '2';
                $link_next = $this->createUrl('answers').'?category_id='.$this->_active_category_id.'&AskAnswer_page='.$next;
                ...
                $str.=          isset($_GET['AskAnswer_page'])?$_GET['AskAnswer_page']:'1';
                $str.=          '<span class="caret"></span>
                            </button>
                            <ul class="dropdown-menu" id="dropdown-page" role="menu">';
                                for($i=1;$i<=$count_page;$i++)
                                {
                                    $str.= '<li><a href="'.$this->createUrl('answers').'?category_id='.$this->_active_category_id.'&AskAnswer_page='.$i.'">'.$i.'</li>';
                                }
                $str.=     '</ul>
