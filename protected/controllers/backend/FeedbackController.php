...
class FeedbackController extends ModuleController
{
    ...
    public function actionIndex()
    {
        ...
        $dataProducts = Feedback::getFeedbackProvider($this->count, $date_from, $date_to, $status, 't.sort', 'ASC', $tree_id);
...
//        $categories=Yii::app()->db->createCommand("SELECT `t`.`id`,`t`.`title`,`t`.`level` FROM `".$model->tableName()."` `t` WHERE `t`.`status`!=3 and `t`.`language_id`='".$this->getCurrentLanguage()->id."' ORDER BY  t.`root`,t.`lft`")->queryAll();
