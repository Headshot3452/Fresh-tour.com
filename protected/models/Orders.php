...
class Orders extends Model
{
    ...
    const ORDER_DELIVERY_TO_ADDRESS=0;//доставка по адресу
    ...
    public function rules()
    {
        ...
        return array(
            array('user_id, count, sum, type_delivery, type_payments, status', 'required'),
            array('type_delivery, type_payments, paid, status', 'numerical', 'integerOnly'=>true),
            array('sum_delivery', 'numerical'),
            ...
            array('address_text,comment', 'safe'),
    ...
    protected function afterFind()
    {
        ...
        $this->f_sum = Yii::app()->format->formatNumber($this->sum);//сумма заказа
        ...
        if($this->status != self::STATUS_COMPLETED && $this->status > 0)
        {
            ...
            $hours = round(($sec%86400)/3600);
            $min = round((($sec%86400)%3600)/60);
    ...
    {
        $array = array(self::STATUS_NOT_DELIVERED=>'Не доставлен',
    ...
    public static function createOrder($user_id,$products,$payment_type,$address_type,$address_id=null)
    {
        $count=0;
        $sum=0;
        $orderItems=array();
        ...
        $order->type_payments=$payment_type;
        $order->type_delivery=$address_type;


        if ($order->type_delivery==self::ORDER_DELIVERY_TO_ADDRESS) //если доставка на адресс
        {
            $user_address=$order->getInstanceRelation('address')->findByPk($address_id);
            if ($user_address)
            {
                $order->address_id=$user_address->id;
                $order->address_text=serialize($user_address->attributes);
            }

            if ($order->sum<$order->getDeliveryLimit())
            {
                $order->sum_delivery=$order->getDeliveryPrice();
            }
        }

//        $order->type_delivery=$type_delivery;
        $order->email=$info['email'];
        $order->phone=$info['phone'];
        $order->comment=$info['comment'];
        $order->info=serialize($info);
    ...
    {
        $array=array(self::ORDER_DELIVERY_NOT_ADDRESS=>'Самовывоз',
                    self::ORDER_DELIVERY_TO_ADDRESS=>'Доставка на адрес',
        );
    ...
    {
        $array=array(self::ORDER_PAYMENT_CASH=>'Наличные',
        );
