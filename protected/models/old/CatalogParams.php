...
class CatalogParams extends Model
{
	...
	public function relations()
	{
		...
		return array(
			...
			'catalogParamsVals' => array(self::HAS_MANY, 'CatalogParamsVal', 'params_id'),
    ...
    {
        return self::model()->with(array('catalogParams','catalogParamsVals'))->findAllByAttributes(array('catalog_tree_id'=>$tree_ids),array('order'=>'t.`sort`'));
    }
    ...
    public function getSubForm($key,$form,$tree_id=null,$child_html='')
    {
        ...
        echo '<div class="parent">
                <div class="item">
            '.$form->textField($this,'['.$key.']title').'
            '.$form->error($this,'title').'
            '.$form->hiddenField($this,'['.$key.']type',array('value'=>0));
        if ($this->catalog_tree_id==$tree_id)
        {
//            echo '<a href="javascript:void(0)" class="add-sub-form-child"><i class="glyphicon glyphicon-plus-sign"></i></a>
//                     <a href="javascript:void(0)" class="del-form"><i class="glyphicon glyphicon-remove-circle"></i></a>';
            echo '<a href="javascript:void(0)" class="add-sub-form-child">'.Yii::t('app','Add parameter').'</a>
                     <a href="javascript:void(0)" class="del-form"><span class="icon-admin-delete"></span></a>';
        }

        echo '<div class="child">'.$child_html.'</div>
             </div>
        </div>';
    ...
    public function getSubFormChild($key,$form,$tree_id=null)
    {
        ob_start();
        echo '<div class="children" id="'.$key.'">
            <div class="item">
            '.$form->textField($this,'['.$key.']title').'
            '.$form->error($this,'title').'
            '.$form->hiddenField($this,'['.$key.']parent_id').'
            '.$form->dropDownList($this,'['.$key.']type',self::getType()).'
            '.$form->error($this,'type').'
            '.$form->dropDownList($this,'['.$key.']unit',self::getUnitType()).'
            '.$form->error($this,'unit').'
            '.$form->checkBox($this, '['.$key.']status').'
            '.$form->label($this,'['.$key.']status');;


        if ($this->catalog_tree_id==$tree_id)
        {
            echo '<img class="sort pull-right" src="/images/drag_drop.png" alt="Тяни меня" title="Тяни меня"/><a href="javascript:void(0)" class="del-form"><span class="icon-admin-delete"></span></a>
            <div class="values" '.(($this->type<self::TYPE_SELECT) ? 'style="display:none;"' : '' ).'>' ;
            echo '<a href="javascript:void(0)" class="add-value">'.Yii::t('app','Value param').'</a>
                    <div class="value">';
            $values=$this->getValues();

            foreach ($values as $value)
            {
                $this->getValueForm($value,$form);
            }
            echo '</div>
                </div>
                ';
    ...
    public function getValueForm($value,$form)
    {
        echo '<div class="item">';
        echo $form->textField($value,'['.$this->id.'][values]['.$value->id.']value').
