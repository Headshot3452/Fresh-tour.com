...
<?php
        if(!empty($model->module->module_id))
        {
            ...
            {
                $data_tree = $new_model->active()->findAll('level = :level', array('level' => 1));
            }
        }
...
?>
            <div class="form-group">
                <div class="label-block"><?php echo Yii::t('app','Module Tree'); ?>:</div>
                    <?php echo $form->dropDownList($model, 'module[tree_id]', CHtml::listData($data_tree,'id','title'),array('prompt'=>Yii::t('app','Select the link module level'))); ?>
...
<?php
            ...
            {
                $widget->getForm($widget->id,$form,$all_widgets,$blocks_layout);
            }
?>
        </div>
        <div>
            <a href="javascript:void(0)" class="add_widget">
                <?php echo Yii::t('app','Add a widget');?>
            </a>
        </div>
    </div>
<?php
