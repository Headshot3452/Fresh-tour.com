<?php
    ...
    {
        $sale_price = $data->getSalePrice($data->price_eur, $data->sale_info);
    }
...
<?php
            $ico = $data->price ? 'USD' : 'EUR';

            if($this->currency_ico["format"] == 1)
            {
                $format = 2;
                $data->price = $data->price ? $data->price : $data->price_eur;
            }
            else
            {
                $sale_price = Yii::app()->format->formatNumber(round(str_replace(" ", "", $sale_price)));
                $data->price = $data->price ? round($data->price) : round($data->price_eur);
                $format = 0;
            }
